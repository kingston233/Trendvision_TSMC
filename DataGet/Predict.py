...
def get_latest_data(start_from_year=True):
    ...
    功能說明:
    - 修正：使用本地 CSV 資料，不再依賴 Yahoo Finance API
    - 從 Data 目錄下的最新時間戳記資料夾讀取資料
    - 確保有足夠的資料進行技術指標計算
    - 包含開盤價、最高價、最低價、收盤價、成交量等資訊
    
    Args:
        start_from_year (bool): 保留參數以維持相容性，但不再使用
    ...
    try:
        ...
        data_dir = os.path.join(current_dir, '../Data')
        if not os.path.exists(data_dir):
            print(f"錯誤: Data 資料夾不存在: {data_dir}")
            return None
        ...
        print(f"   使用資料夾: {folder_path}")
        print(f"   時間戳記: {timestamp}")
        
        # 嘗試載入資料檔案
        # 優先使用 tsmc_technical_data.csv (包含 2025 年資料)
        data_file = os.path.join(folder_path, 'tsmc_technical_data.csv')
        ...
        print(f"錯誤: 資料載入失敗: {e}")
        print("   建議解決方案:")
        print("   1. 確認 Data 資料夾存在")
        print("   2. 執行 Get2330.py 抓取最新資料")
        print("   3. 檢查資料檔案格式是否正確")
        import traceback
        traceback.print_exc()
        ...
        if previous_close == 0:
            try:
                ...
                if len(recent_history) >= 2:
                    previous_close = recent_history['Close'].iloc[-2]
            except:
                pass

        # 計算漲跌停價 (台股 10%)
        limit_up = previous_close * 1.10 if previous_close > 0 else 0
        limit_down = previous_close * 0.90 if previous_close > 0 else 0
